...
public class Movement {
    ...
    public void decelerate(Direction direction, float tempSpeed, float tempAcceleration) {
        moveListeners.removeIf(actionListener -> actionListener instanceof DecelerationWorker && ((DecelerationWorker) actionListener).direction.equals(direction));
        ...
        for (ActionListener actionListener : moveListeners) {
            if (actionListener instanceof DecelerationWorker) {
                ((DecelerationWorker) actionListener).acceleration *=
                        ((DecelerationWorker) actionListener).acceleration >= 0 ? 1 / DAMPEN_FACTOR.getValue() : DAMPEN_FACTOR.getValue();
            }
        }
    }
